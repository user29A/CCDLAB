...
array<String^>^ UVIT::Open_CentroidFile(System::String ^FullPathName)
{
	CENTROID_PRECISION = 1;
	RANGE = gcnew array<int>(4);
	array<String^>^ res = gcnew array<String^>(12);
	FileStream^ fs = gcnew FileStream(FullPathName,System::IO::FileMode::Open,System::IO::FileAccess::Read);
	...
	if (res[0]->Compare(res[0],"3x3_Cross") == 0)
	{
	...
	if (res[0]->Compare(res[0],"3x3_Square") == 0)
	{
		TBTS_CENTROIDS = gcnew array<double,2>(7,N);
		for (int i = 0; i < N; i++)//for 3x3 Cross
		{
			str = sr->ReadLine();
			ind = str->IndexOf('	');
	...
	if (res[0]->Compare(res[0],"3x3_Gaussian") == 0)
	{
		TBTQ_CENTROIDS = gcnew array<double,2>(7,N);
		for (int i = 0; i < N; i++)//for 3x3 Cross
		{
			str = sr->ReadLine();
			ind = str->IndexOf('	');
	...
	if (res[0]->Compare(res[0],"5x5_Cross") == 0)
	{
		FBFC_CENTROIDS = gcnew array<double,2>(7,N);
		for (int i = 0; i < N; i++)//for 3x3 Cross
		{
			str = sr->ReadLine();
			ind = str->IndexOf('	');
	...
	if (res[0]->Compare(res[0],"5x5_Square") == 0)
	{
		FBFS_CENTROIDS = gcnew array<double,2>(7,N);
		for (int i = 0; i < N; i++)//for 3x3 Cross
		{
			str = sr->ReadLine();
			ind = str->IndexOf('	');
...
array<unsigned __int32,2>^ UVIT::Get_Image(System::String ^type)
{
	...
	array<unsigned __int32,2>^ image = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*(int)CP,(RANGE[3]-RANGE[2]+1)*(int)CP);
	if (type->Compare(type,"3x3_Cross") == 0)
	{
		int i = 0;
		for (i; i < TBTC_CENTROIDS->GetLength(1); i++)
		{
			try
			{
			...
			{
				/*CCDLAB::NotifyDlg^ note = gcnew CCDLAB::NotifyDlg();
	...
	if (type->Compare(type,"3x3_Square") == 0)
	{
		int i = 0;
		for (i; i < TBTS_CENTROIDS->GetLength(1); i++)
		{
			try
			{
	...
	if (type->Compare(type,"3x3_Gaussian") == 0)
	{
	...
	if (type->Compare(type,"5x5_Cross") == 0)
	{
	...
	if (type->Compare(type,"5x5_Square") == 0)
	{
		int i = 0;
		try
		{
			for (i; i < FBFS_CENTROIDS->GetLength(1); i++)
			{
...
array<unsigned __int32,2>^ UVIT::Get_FromOnlyImageMode(System::String ^type)
{
	if (type->Compare(type,"3x3_Cross") == 0)
	{
		return TBTC_IMAGE;
	}
	else if (type->Compare(type,"3x3_Square") == 0)
	{
		return TBTS_IMAGE;
	}
	else if (type->Compare(type,"3x3_Gaussian") == 0)
	{
		return TBTQ_IMAGE;
	}
	else if (type->Compare(type,"5x5_Cross") == 0)
	{
		return FBFC_IMAGE;
	}
	else if (type->Compare(type,"5x5_Square") == 0)
	{
		return FBFS_IMAGE;
	}
	else
...
void UVIT::Save_Centroids(String^ FullPathName, String^ type, String^ arraybgname, bool activeBG)
{
	FileStream^ fs = gcnew FileStream(FullPathName,System::IO::FileMode::Create,System::IO::FileAccess::ReadWrite);
	...
	else if (arraybgname->Compare(arraybgname,"NA") == 0)
...
bool UVIT::Centroid(int Precision, cli::array<int> ^Range, int MinPixel, int MaxPixel, int MinShape, int MaxShape,int ScalarBackGround, cli::array<bool,1> ^AlgTypes, cli::array<unsigned __int16,2>^ ArrayBackGround,  BackgroundWorker^ worker, bool imageonly, bool ActiveBG, int  CornerThresh, double BGOffset,JPWaitBar::WaitBar^ wb)
{
	...
	ArrayList^ tbtc = gcnew ArrayList();
	ArrayList^ tbts = gcnew ArrayList();
	ArrayList^ tbtq = gcnew ArrayList();
	ArrayList^ fbfc = gcnew ArrayList();
	ArrayList^ fbfs = gcnew ArrayList();
	...
	for (int n = 0; n < NUM_FILES; n++)
	{
		...
		raw = FITSImage::ReadImageArrayOnly(FULL_FILE_LIST[n], RANGE, true);
		...
		for (int i = 2; i < raw->GetLength(0) - 2; i++)
		{
			for (int j = 2; j < raw->GetLength(1) - 2; j++)//loops automatically do the rastering, inside the bounds at 5x5
			{
				...
				{	
					array<double,2>^ tbtv = gcnew array<double,2>(3,3);//TBT values
					...
					shape = JPMath::Sum(JPMath::MatrixSubScalar(tbtv, fcmin, false), false);
					if (shape < MIN_SHAPE || shape > MAX_SHAPE)//Sum energy check
					{
					...
					tbtv = JPMath::MatrixSubScalar(tbtv, fcmin + BGOffset, false);//subtract 4 corner min from entire array
					//Array should now be set so background = -BGOffset

					ix = double(i);
					iy = double(j);
					fx = (tbtv[2,0] - tbtv[0,0] + tbtv[2,1] - tbtv[0,1] + tbtv[2,2] - tbtv[0,2])/JPMath::Sum(tbtv, false);
					fy = (tbtv[0,2] - tbtv[0,0] + tbtv[1,2] - tbtv[1,0] + tbtv[2,2] - tbtv[2,0])/JPMath::Sum(tbtv, false);
				...
				{
					array<double,2>^ fbfv2 = gcnew array<double,2>(5,5);//FBF values
					...
					shape = JPMath::Sum(JPMath::MatrixSubScalar(fbfv2, fcmin, false), false);
					if (shape < MIN_SHAPE || shape > MAX_SHAPE)//Sum energy check
					{
						goto next5;//skip out this algorithms only
					}

					fbfv2 = JPMath::MatrixSubScalar(fbfv2, fcmin + BGOffset, false);//subtract 4 corner min from entire array
						//Array should now be set so background = -BGOffset

					ix = double(i);
					iy = double(j);
					fx = (fbfv2[3,0] + 2*fbfv2[4,0] + fbfv2[3,1] + 2*fbfv2[4,1] + fbfv2[3,2] + 2*fbfv2[4,2] + fbfv2[3,3] + 2*fbfv2[4,3] + fbfv2[3,4] + 2*fbfv2[4,4] - (2*fbfv2[0,0] + fbfv2[1,0] + 2*fbfv2[0,1] + fbfv2[1,1] + 2*fbfv2[0,2] + fbfv2[1,2] + 2*fbfv2[0,3] + fbfv2[1,3] + 2*fbfv2[0,4] + fbfv2[1,4]))/JPMath::Sum(fbfv2, false);
					fy = (fbfv2[0,3] + 2*fbfv2[0,4] + fbfv2[1,3] + 2*fbfv2[1,4] + fbfv2[2,3] + 2*fbfv2[2,4] + fbfv2[3,3] + 2*fbfv2[3,4] + fbfv2[4,3] + 2*fbfv2[4,4] - (2*fbfv2[0,0] + fbfv2[0,1] + 2*fbfv2[1,0] + fbfv2[1,1] + 2*fbfv2[2,0] + fbfv2[2,1] + 2*fbfv2[3,0] + fbfv2[3,1] + 2*fbfv2[4,0] + fbfv2[4,1]))/JPMath::Sum(fbfv2, false);
	...
	if (N > 0)
	{
		...
		for (int i = 0; i < N; i++)
		{
			TBTS_CENTROIDS[0,i] = System::Convert::ToDouble(tbts[i*7]);
			TBTS_CENTROIDS[1,i] = System::Convert::ToDouble(tbts[i*7+1]);
			TBTS_CENTROIDS[2,i] = System::Convert::ToDouble(tbts[i*7+2]);
			TBTS_CENTROIDS[3,i] = System::Convert::ToDouble(tbts[i*7+3]);
			TBTS_CENTROIDS[4,i] = System::Convert::ToDouble(tbts[i*7+4]);
			TBTS_CENTROIDS[5,i] = System::Convert::ToDouble(tbts[i*7+5]);
			TBTS_CENTROIDS[6,i] = System::Convert::ToDouble(tbts[i*7+6]);
		}
		tbts->Clear();//tbts->TrimToSize();
	}
	else
	{
		TBTS_CENTROIDS = nullptr;
	}
	...
	if (N > 0)
	{
		...
		for (int i = 0; i < N; i++)
		{
			TBTQ_CENTROIDS[0,i] = System::Convert::ToDouble(tbtq[i*7]);
			TBTQ_CENTROIDS[1,i] = System::Convert::ToDouble(tbtq[i*7+1]);
			TBTQ_CENTROIDS[2,i] = System::Convert::ToDouble(tbtq[i*7+2]);
			TBTQ_CENTROIDS[3,i] = System::Convert::ToDouble(tbtq[i*7+3]);
			TBTQ_CENTROIDS[4,i] = System::Convert::ToDouble(tbtq[i*7+4]);
			TBTQ_CENTROIDS[5,i] = System::Convert::ToDouble(tbtq[i*7+5]);
			TBTQ_CENTROIDS[6,i] = System::Convert::ToDouble(tbtq[i*7+6]);
		}
		tbtq->Clear();tbtq->TrimToSize();
	}
	else
	{
		TBTQ_CENTROIDS = nullptr;
	}
	...
	if (N > 0)
	{
		...
		for (int i = 0; i < N; i++)
		{
			FBFC_CENTROIDS[0,i] = System::Convert::ToDouble(fbfc[i*7]);
			FBFC_CENTROIDS[1,i] = System::Convert::ToDouble(fbfc[i*7+1]);
			FBFC_CENTROIDS[2,i] = System::Convert::ToDouble(fbfc[i*7+2]);
			FBFC_CENTROIDS[3,i] = System::Convert::ToDouble(fbfc[i*7+3]);
			FBFC_CENTROIDS[4,i] = System::Convert::ToDouble(fbfc[i*7+4]);
			FBFC_CENTROIDS[5,i] = System::Convert::ToDouble(fbfc[i*7+5]);
			FBFC_CENTROIDS[6,i] = System::Convert::ToDouble(fbfc[i*7+6]);
		}
		fbfc->Clear();fbfc->TrimToSize();
	}
	else
	{
		FBFC_CENTROIDS = nullptr;
	}
	...
	if (N > 0)
	{
		...
		for (int i = 0; i < N; i++)
		{
			FBFS_CENTROIDS[0,i] = System::Convert::ToDouble(fbfs[i*7]);
			FBFS_CENTROIDS[1,i] = System::Convert::ToDouble(fbfs[i*7+1]);
			FBFS_CENTROIDS[2,i] = System::Convert::ToDouble(fbfs[i*7+2]);
			FBFS_CENTROIDS[3,i] = System::Convert::ToDouble(fbfs[i*7+3]);
			FBFS_CENTROIDS[4,i] = System::Convert::ToDouble(fbfs[i*7+4]);
			FBFS_CENTROIDS[5,i] = System::Convert::ToDouble(fbfs[i*7+5]);
			FBFS_CENTROIDS[6,i] = System::Convert::ToDouble(fbfs[i*7+6]);
		}
		fbfs->Clear();//fbfs->TrimToSize();
	}
	else
	{
		FBFS_CENTROIDS = nullptr;
	}
	return !cancel;//return true if finished (!cancel will = true, since cancel will be false here)
	//System::GC::Collect();
}

array<double,2>^ UVIT::Read_Raw_File(int filenum, int BackGround)
{
	array<int,2>^ result = gcnew array<int,2>(512,512);
	FileStream^ fs = gcnew FileStream(FULL_FILE_LIST[filenum],System::IO::FileMode::Open,FileAccess::Read);
...
array<double,2>^ UVIT::Read_Raw_File(int filenum, int BackGround, array<int,2>^ Range /*Range is ZERO based!*/)
{
	FileStream^ fs = gcnew FileStream(FULL_FILE_LIST[filenum],System::IO::FileMode::Open,FileAccess::Read);
	BinaryReader^ br = gcnew BinaryReader(fs);
	int W = Range[1,0] - Range[0,0] + 1;
	int H = Range[1,1] - Range[0,1] + 1;
	array<int,2>^ result = gcnew array<int,2>(W,H);
	br->BaseStream->Seek(((Range[0,1] )*512*2 + (Range[0,0] )*2), ::IO::SeekOrigin::Begin);
	for (int i = 0; i < H; i++)
	{
		for (int j = 0; j < W; j++)
		{
		...
		br->BaseStream->Seek((512 - Range[1,0] + Range[0,0] - 1)*2, ::IO::SeekOrigin::Current);
	}
	br->Close();
	fs->Close();
	return nullptr;
}
array<double,2>^ UVIT::Read_Raw_File(int filenum, array<int,2> ^ArrayBackGround)
{
	array<int,2>^ result = gcnew array<int,2>(512,512);
	FileStream^ fs = gcnew FileStream(FULL_FILE_LIST[filenum],System::IO::FileMode::Open,FileAccess::Read);
	BinaryReader^ br = gcnew BinaryReader(fs);
...
array<double,2>^ UVIT::Read_Raw_File(int filenum, array<int,2> ^ArrayBackGround, array<int,2>^ Range /*Range is ZERO based!*/)
{
	FileStream^ fs = gcnew FileStream(FULL_FILE_LIST[filenum],System::IO::FileMode::Open,FileAccess::Read);
	BinaryReader^ br = gcnew BinaryReader(fs);
	int W = Range[1,0] - Range[0,0] + 1;
...
/*array<double,2>^ UVIT::Read_Raw_File(String^ FullFile)//this is for external interface to JPFITSImage file loading in CCDLAB.cpp
{//no longer need because now writing .raw files as fits files, with .raw ending

	//FileStream^ fs = gcnew FileStream(FullFile,System::IO::FileMode::Open,FileAccess::Read);
	//BinaryReader^ br = gcnew BinaryReader(fs);

	JPFITSImage^ f = gcnew JPFITSImage(FullFile,false);
	return f->Image;

	//array<double,2>^ result = gcnew array<double,2>(512,512);
	//for (int i = 0; i < 512; i++)
	//{
	//	for (int j = 0; j < 512; j++)
	//	{
	//		result[j,i] = double(br->ReadUInt16());
	//	}
	//}
	//br->Close();
	//fs->Close();
...
//array<double,2>^ UVIT::Read_Raw_File(String^ FullFile, array<int,2>^ Range /*Range is ZERO based!*/)//this is for external interface to JPFITSImage file loading in CCDLAB.cpp
/*{
	FileStream^ fs = gcnew FileStream(FullFile,System::IO::FileMode::Open,FileAccess::Read);
	BinaryReader^ br = gcnew BinaryReader(fs);
	int W = Range[1,0] - Range[0,0] + 1;
...
/*void UVIT::Write_Raw_File(System::String ^FullFilePath, cli::array<double,2> ^image)
{
	/*FileStream^ fsb = gcnew FileStream(FullFilePath, ::IO::FileMode::Create );
	BinaryWriter^ bw = gcnew BinaryWriter(fsb);
	for (int i = 0; i < 512; i++)//write the image now that done reading
		for (int j = 0; j < 512; j++)
		{
			bw->Write((unsigned __int16)image[j,i]);
		}
	bw->Close();
	fsb->Close();
	...
	JPFITSImage^ f = gcnew JPFITSImage(FullFilePath,image,false);
	f->WriteImage(TypeCode::UInt16);
}*/

UVIT::UVIT(array<System::String^,1>^ FullFileList)
{
	FULL_FILE_LIST = FullFileList;
	FILE_LIST = gcnew array<String^>(FullFileList->Length);
...
void UVIT::InitImages(array<bool>^ types)
{
	int CP = CENTROID_PRECISION;
	if (types[0] == true)
	{
		TBTC_IMAGE = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*CP,(RANGE[3]-RANGE[2]+1)*CP);
	}
	if (types[1] == true)
	{
		TBTS_IMAGE = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*CP,(RANGE[3]-RANGE[2]+1)*CP);
	}
	if (types[2] == true)
	{
		TBTQ_IMAGE = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*CP,(RANGE[3]-RANGE[2]+1)*CP);
	}
	if (types[3] == true)
	{
		FBFC_IMAGE = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*CP,(RANGE[3]-RANGE[2]+1)*CP);
	}
	if (types[4] == true)
	{
		FBFS_IMAGE = gcnew array<unsigned __int32,2>((RANGE[1]-RANGE[0]+1)*CP,(RANGE[3]-RANGE[2]+1)*CP);
	}
}
